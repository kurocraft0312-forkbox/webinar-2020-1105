...
def main():
    ...
    for keyword in keyword_list:
        ...
        while True:
            ...
            if len(result['items']) <= 50:
                ...
                break
            # もしも動画数が50件以上の場合はページ送りのトークン(result['nextPageToken']を変数nextpagetokenに設定する
            else:
                for i in result['items']:
                    data.append([i['id']['videoId'],i['snippet']['publishedAt'],i['snippet']['title'],keyword])
    ...
    df_data =  df_data.query('viewcount>=10000')
    
    df_data = df_data[['search_day','keyword','title','url','viewcount']]
    df_data['viewcount'] =  df_data['viewcount'].astype(str)

    st.title('Youtubeで24時間以内に1万回以上再生された動画リスト')
    st.write('キーワード：Python、マコなり社長、ヒカキン、朝倉みくる、てんちむ')

    # 開くImageの定義
    image = Image.open('film-589490_640.jpg')

    st.image(image, caption='source: https://pixabay.com/illustrations/film-filmstrip-you-tube-you-tube-589490/',
           use_column_width=True)

    st.write(df_data)

    df_vc = df_data.loc[:,['keyword', 'viewcount']].set_index('keyword')
